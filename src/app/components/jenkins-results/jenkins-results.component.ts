...
@Component({
  ...
  styleUrls: ["./jenkins-results.component.scss"],
  standalone: true,
...
export class JenkinsResultsComponent implements OnInit, OnDestroy {
  ...
  automationTesterSelection: Record<number, number | null> = {};

  manualTesterSelection: Record<number, number | null> = {};

  projectSelection: Record<number, number | null> = {};
  ...
  @ViewChild("overallChartCanvas") chartRef!: ElementRef<HTMLCanvasElement>;
  overallChart!: Chart;
  ...
  loadJenkinsData(): void {
    this.loading = true;
    try {
      this.loadJenkinsResults(), this.loadJenkinsStatistics();
      setTimeout(() => {
  ...
  loadJenkinsResults(): void {
    // Use filtered results if filters are applied
    const hasFilters =
      this.selectedProjectId ||
      this.selectedAutomationTesterId ||
      this.selectedJobFrequency ||
      this.selectedStatus;
  ...
  loadProjects(): void {
    this.apiService.getProjects().subscribe(
      ...
      (error) =>
        console.error("Error loading projects with Jenkins results:", error),
    );
  }
  ...
  private getTesterIdSafely(
    tester: Tester | number | null | undefined,
  ): number | null {
    if (!tester) {
      return null;
    }
  ...
  private getProjectIdSafely(
    project: Project | number | null | undefined,
  ): number | null {
    if (!project) {
      return null;
    }
  ...
  selectJob(job: JenkinsResult): void {
    ...
    this.originalAutomationTester = this.getTesterIdSafely(
  ...
  checkSaveButtonState(): void {
    ...
    const automationTesterChanged =
      this.automationTesterSelection[this.selectedJob.id] !==
      this.originalAutomationTester;
    const manualTesterChanged =
      this.manualTesterSelection[this.selectedJob.id] !==
      this.originalManualTester;
    const projectChanged =
      this.projectSelection[this.selectedJob.id] !== this.originalProject;

    const hasNotes = this.jobNotes && this.jobNotes.trim().length > 0;
    const hasAutomationTester =
      this.automationTesterSelection[this.selectedJob.id] !== null;
    const hasManualTester =
      this.manualTesterSelection[this.selectedJob.id] !== null;
  ...
  saveAllData(): void {
    ...
    this.apiService
      .saveJenkinsJobData(this.selectedJob.id, requestData)
      .subscribe(
        (response) => {
          ...
          if (this.selectedJob) {
            ...
            if (requestData.automationTesterId) {
              const tester = this.testers.find(
                (t) => t.id === requestData.automationTesterId,
              );
              if (tester) {
            ...
            if (requestData.manualTesterId) {
              const tester = this.testers.find(
                (t) => t.id === requestData.manualTesterId,
              );
              if (tester) {
  ...
  showSuccessToast(message: string, isError = false): void {
    ...
    this.apiService.getJenkinsTestCases(resultId).subscribe(
      (testCases: JenkinsTestCase[]) => {
        ...
        if (this.selectedJobTestCases.length === 0 && this.selectedJob) {
          this.loadDetailedTestNGResults(
            this.selectedJob.jobName,
            this.selectedJob.buildNumber,
          );
        } else {
  ...
  copyStackTrace(): void {
    if (this.selectedStackTrace) {
      navigator.clipboard
        .writeText(this.selectedStackTrace)
        .then(() => {
        ...
        .catch(() => {
          ...
          try {
            ...
            this.showSuccessToast(
              "Failed to copy stack trace. Please copy manually.",
              true,
            );
          }
  ...
  applyFilters(): void {
    ...
    if (this.selectedStatus) {
      filtered = filtered.filter(
        (result) => result.buildStatus === this.selectedStatus,
      );
    }
    ...
    if (this.passPercentageThreshold) {
      if (this.passPercentageOperator === "gte") {
        filtered = filtered.filter(
          (r) => this.getPassPercentage(r) >= this.passPercentageThreshold,
        );
      } else {
        filtered = filtered.filter(
          (r) => this.getPassPercentage(r) <= this.passPercentageThreshold,
  ...
  applyTestCaseFilters(): void {
    ...
    if (this.selectedTestCaseStatus) {
      filtered = filtered.filter(
        (testCase) => testCase.status === this.selectedTestCaseStatus,
      );
    }
  ...
  sortTestCases(column: string): void {
    if (this.currentTestCaseSort.column === column) {
      this.currentTestCaseSort.direction =
        this.currentTestCaseSort.direction === "asc" ? "desc" : "asc";
    } else {
  ...
  getTestCaseSortIcon(column: string): string {
    ...
    return this.currentTestCaseSort.direction === "asc"
      ? "fas fa-sort-up"
      : "fas fa-sort-down";
  }
  ...
  getSkippedTestCasesCount(): number {
    return this.filteredTestCases.filter((tc) => tc.status === "SKIPPED")
      .length;
  }
  ...
  getProjectName(result: JenkinsResult): string {
    ...
    return typeof result.project === "object"
      ? result.project.name
      : "Unknown Project";
  }
  ...
  assignTesters(result: JenkinsResult): void {
    ...
    this.apiService
      .assignTestersToJenkinsResult(result.id, automationId, manualId)
      .subscribe(
        () => {
          if (automationId) {
  ...
  saveJobNotes(): void {
    ...
    const notes = {
      bugsIdentified: this.jobNotes,
      failureReasons: this.jobNotes,
    };
